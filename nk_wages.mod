...
i= phi_pi * pi_p + phi_y * yhat + phi_w * pi_w +nu;
...
stoch_simul(order = 1,irf=15,nograph,noprint) y_gap w_real c pi_w pi_p w_gap i; 
oo_sticky_wages=oo_;

%flex wage case
set_param_value('theta_w',0.00000001)
set_param_value('theta_p',3/4)
stoch_simul(order = 1,irf=15) y_gap w_real c pi_w pi_p w w_gap i;
...
subplot(3,2,2)
plot(1:options_.irf,oo_sticky_wages.irfs.pi_w_eps_nu,'-d',1:options_.irf,oo_flexible_wages.irfs.pi_w_eps_nu,'-s')
title('Wage Inflation')

subplot(3,2,3)
plot(1:options_.irf,oo_sticky_wages.irfs.pi_p_eps_nu,'-d',1:options_.irf,oo_flexible_wages.irfs.pi_p_eps_nu,'-s')
title('Price Inflation')

subplot(3,2,4)
plot(1:options_.irf,oo_sticky_wages.irfs.w_real_eps_nu,'-d',1:options_.irf,oo_flexible_wages.irfs.w_real_eps_nu,'-s')
title('Real Wage')

subplot(3,2,5)
plot(1:options_.irf,oo_sticky_wages.irfs.w_gap_eps_nu,'-d',1:options_.irf,oo_flexible_wages.irfs.w_gap_eps_nu,'-s')
title('Wage Gap')

subplot(3,2,5)
plot(1:options_.irf,oo_sticky_wages.irfs.i_eps_nu,'-d',1:options_.irf,oo_flexible_wages.irfs.i_eps_nu,'-s')
title('i')

exportgraphics(gcf,'nk_irf_eps_nu.png','Resolution',300)
...
varlist = {'y_gap', 'w_real', 'c', 'pi_w', 'pi_p', 'w_gap', 'i'};
labelList = {'Output Gap', 'Real Wage', 'Consumption', 'Wage Inflation', 'Price Inflation', 'Wage Gap' 'Interest Rate'};
...
exportgraphics(gcf,'NK_irf_eps_nu_DR.png','Resolution',300)
